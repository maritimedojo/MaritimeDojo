...
namespace MaritimeDojo.Services
{
    ...
    {
        private readonly GetAllReservationsQuery queryAll;
        private readonly GetReservationByIdQuery queryById;
        private readonly AddReservationQuery queryAdd;
        private readonly DeleteReservationQuery queryDelete;
        private readonly GetAllLecturersQuery queryAllLecturers;
        private readonly GetAllLectureHallsQuery queryAllLectureHalls;
        ...
        public ReservationsService(GetAllReservationsQuery queryAll, GetReservationByIdQuery queryById, AddReservationQuery queryAdd, DeleteReservationQuery queryDelete, GetAllLecturersQuery queryAllLecturers, GetAllLectureHallsQuery queryAllLectureHalls)
        {
            ...
            this.queryAllLecturers = queryAllLecturers;
            this.queryAllLectureHalls = queryAllLectureHalls;
        }
        ...
        {
            return Mapper.Map<IEnumerable<ReservationItem>>(queryAll.Execute().ToList());
        }
        ...
        {
            return Mapper.Map<ReservationItem>(queryById.Execute(id));
        }
        ...
        {
            if (!queryAllLectureHalls.Execute().Any(p => p.Number == hallNumber))
            {
                throw new ArgumentException("Given hall does not exist");
            }

            var reservations =
                queryAll.Execute().Where(p => p.Hall.Number == hallNumber).Where(p => p.From.Date == day.Date).OrderBy(p => p.From.Hour);
        ...
        public IEnumerable<HallFreeHoursStatisticsItem> GetHallsFreeHoursByDay(DateTime day)
        {
            ...
            var occupiedHallsStatistics =
                queryAll.Execute().Where(p => p.From.Date == day.Date).GroupBy(p => p.Hall.Number).Select(p => new HallFreeHoursStatisticsItem
                {
                    HallNumber = p.Key,
                    FreeHoursNumber = p.Sum(r => r.To.Hour - r.From.Hour)
                });

            result.AddRange(queryAllLectureHalls.Execute().Select(p => new HallFreeHoursStatisticsItem
            {
                HallNumber = p.Number,
